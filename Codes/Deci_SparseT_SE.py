...
def logZ(m,Delta,rho,Theta):
    ...
    fct4 = 0.5*rho*m*m/Delta
    return (1-Theta)*(rho * scipy.integrate.quad(fct1, -6, 6)[0] + (1-rho) * scipy.integrate.quad(fct2, -6, 6)[0]) + Theta*(fct3+fct4) - m*m*m/(3*Delta)

def uninf_SE(Tmin,Tmax,NT,rho,Doverrho2, max_iter=10000, tol=1e-6):
    ...
    for i_T,Theta in enumerate(np.linspace(Tmin,Tmax,NT)):
        ...
        for i in range(max_iter):
            m_old = m_SE
            m_SE = 0.5*(Theta + (1-Theta)*SE(m_SE,Delta,rho)) + 0.5 * m_SE
...
def inf_SE(Tmin,Tmax,NT,rho,Doverrho2, max_iter=10000, tol=1e-6):
    Delta = rho*rho*Doverrho2
    m_init = rho
    m_tab = np.zeros(NT)
    it_tab = np.zeros(NT)
...
def main():
    ...
    file_name = "DF_Diff_SparseT_SE_DoR2_" + str(Doverrho2) + "_Tm_" + str(Tmin) + "_TM_" + str(Tmax) + "_NT_" + str(NT) + "_r_" + str(rho) + ".xz"
