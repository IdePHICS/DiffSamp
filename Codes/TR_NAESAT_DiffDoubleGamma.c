...
float CumTab[K], wTab0[K+1], wTab[K+1], *Q, *Qhat, htab[K-1], htabK[K], CumTabK[K+1], PhiRS, gamma_;
...
float fFP(float *utab, int d) {
  ...
  if (prod1 +prod2 > 0.0) {
    z = gaussRan();
    prod1 *= exp(gamma_*gamma_ + gamma_*z);
    prod2 *= exp(-(gamma_*gamma_ + gamma_*z));
    h =(prod1-prod2)/(prod1+prod2);
  }
  else {
    if (posNull > negNull)
...
float Z0v(float *utab, int d) {
  ...
  //printf("V: %f \n",  (prod1 + prod2));
  return (exp(gamma_*gamma_ + gamma_*z)*prod1 + exp(-(gamma_*gamma_ + gamma_*z))*prod2);  //NOTE: NOT NORMALIZED
}
...
int main(int argc, char *argv[]) {
  int is, it, start, maxIter, seed, itu;
  float alpha, c, ov, eps_brd, eps_rcs, q1, q1_ave, ov_ave, q1u, ovu, logZ, logZu,logZ_ave, g2, g2min, g2max, g2step;
  char filename[128];// folder_name[20];

  if (argc != 11) {
    fprintf(stderr, "usage: %s folder_name eps_brd eps_rcs alpha g2min g2max g2step start maxIter [seed]\n", argv[0]);
    exit(EXIT_FAILURE);
  }
  //folder_name = argv[1];
  eps_brd = atof(argv[2]);
  ...
  printf("## eps_brd = %.3f   eps_rcs = %.3f   K = %i   N = %i   alpha = %.3f  g2min = %.3f  g2max = %.3f  g2step = %.3f   start = %i   maxIter = %i   seed = %i\n",
	 eps_brd, eps_rcs, K, N, alpha, g2min, g2max, g2step, start, maxIter, seed);
  //printf("# 1:t  2:q1_t\n");
  ...
  sprintf(filename, "%s/dataFP_EB%.3f_ER%.3f_K%i_a%.3f_gm%.3f_gM%.3f_gs%.3f_st%i_s%i.txt", argv[1], eps_brd, eps_rcs, K, alpha, g2min, g2max, g2step, start, seed );
  FILE *data_file = fopen(filename, "w");
  if (data_file == NULL) error("Error: Unable to open file\n");
  fprintf(data_file, "## eps_brd = %.3f   eps_rcs = %.3f   K = %i   N = %i   alpha = %.3f  g2min = %.3f  g2max = %.3f  g2step = %.3f\n",
	 eps_brd, eps_rcs, K, N, alpha, g2min, g2max, g2step);
  fprintf(data_file, "## start = %i   maxIter = %i   seed = %i\n",
	 start, maxIter, seed);
  fprintf(data_file, "#0:gamma2  1:ov_0  2:q1_0  3:logZ_0  4:ov_1  5:q1_1  6:logZ_1  7:delta_ov  8:delta_q1  9:delta_logZ\n");
  for (g2 = g2min; g2 <= g2max; g2 += g2step)
  {
    gamma_ = sqrt(g2);
    initQrandom();
    updateQhat();
    ...
    fprintf(data_file, "%f %f %f %f %f %f %f %f %f %f\n", g2, ovu, q1u, logZu, ov_ave, q1_ave, logZ_ave, ov_ave-ovu, q1_ave-q1u, logZ_ave-logZu);
    fflush(data_file);  
  }
