...
def logZ(m,Delta,gamma2,rho):
    ...
    return rho * scipy.integrate.quad(fct1, -6, 6)[0] + (1-rho) * scipy.integrate.quad(fct2, -6, 6)[0] - m*m*m/(3*Delta)
...
def inf_SE(gmin,gmax,Ng,rho,Doverrho2, max_iter=100000, tol=1e-12):
    Delta = rho*rho*Doverrho2
    m_init = rho
    m_tab = np.zeros(Ng)
    it_tab = np.zeros(Ng)
    ...
    for i_g,gamma2 in enumerate(np.linspace(gmin**2,gmax**2,Ng)):
        ...
        for i in range(max_iter):
            m_old = m_SE
            m_SE = 1.*SE(m_SE,Delta,gamma2,rho) + 0.0 * m_SE
            if (np.abs(m_SE-m_old)/m_SE<tol):
                break
            if  np.abs(m_SE)<1e-50:
                m_SE=0
        ...
        logZ_tab[i_g] = logZ(m_SE,Delta,gamma2,rho)
        it_tab[i_g] = i
        #if (i_g==0): print("m_SE=",m_SE/rho, "it=",i)
    return m_tab,it_tab,logZ_tab
...
def main():
    ...
    keys = [r"$\gamma^2$","step","# iter (uninf)","# iter (inf)", r'$m_{uninf}/\rho$', r'$m_{inf}/\rho$', r'$\delta m/\rho$', r'$\log Z_{uninf}$', r'$\log Z_{inf}$', r'$\Delta/\rho^2$', r"$\beta$", "gmin", "gmax", "rho", "Ng"]
    dict_list=[]
    for i,gamma2 in enumerate(np.linspace(gmin**2,gmax**2,Ng)):
        values = [gamma2,i,it_uninf[i],it_inf[i], m_uninf[i], m_inf[i], diff[i], logZ_un[i],logZ_in[i], Doverrho2, beta, gmin, gmax, rho, Ng]
        dict_list.append(dict(zip(keys, values)))
    df = pd.DataFrame(dict_list)
    #dff = pd.DataFrame(values,keys)

    file_name = "DF_Diff_SparseT_SE_DoR2_" + str(Doverrho2) + "_gmin_" + str(gmin) + "_gmax_" + str(gmax) + "_Ng_" + str(Ng) + "_r_" + str(rho) + ".xz"
